...
var lastSpeakTime
const autoReconnectAvatar = false
const useLocalVideoForIdle =false
const continuousConversation =false
const personalVoiceSpeakerProfileID =''
const customVoiceEndpointId =''
const ttsVoice ='en-US-JennyMultilingualV2Neural'
const sttLocalesSAM = 'en-US,de-DE,es-ES,fr-FR,it-IT,ja-JP,ko-KR,zh-CN'
const enableOyd =false
const azureOpenAIDeploymentName='Avatar'
    ...
    .then(response => {
        if (response.ok) {
            ...
            response.text().then(text => {
                ...
                var sttLocales = sttLocalesSAM.split(',')
...
function connectToAvatarService(peerConnection) {
    ...
    if (azureOpenAIDeploymentName !== '') {
        headers['AoaiDeploymentName'] = azureOpenAIDeploymentName
    }

    if (enableOyd ==true && azureCogSearchIndexName !== '') {
        headers['CognitiveSearchIndexName'] = azureCogSearchIndexName
    }
    ...
    if (customVoiceEndpointId !== '') {
        headers['CustomVoiceEndpointId'] = customVoiceEndpointId
    }

    if (personalVoiceSpeakerProfileID !== '') {
        headers['PersonalVoiceSpeakerProfileId'] = personalVoiceSpeakerProfileID
    }
...
window.microphone = () => {
    ...
    speechRecognizer.recognized = async (s, e) => {
        if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
            ...
            if (continuousConversation == false) {
                document.getElementById('microphone').disabled = true
                speechRecognizer.stopContinuousRecognitionAsync(
                    () => {
                        document.getElementById('microphone').innerHTML = 'Start Microphone'
